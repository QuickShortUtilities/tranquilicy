...
PROGRESS_PATH = r"C:\Users\Gaming PC\musicgen_data\progress.json"


class DashboardProgressCallback(TrainerCallback):
    """Writes step/loss/ETA to the same progress.json the local dashboard polls."""

    def _write(self, stage, current, total, current_item="", **extra):
        data = {
            "stage": stage,
            "current": current,
            "total": total,
        ...
        try:
            tmp = PROGRESS_PATH + f".{os.getpid()}.tmp"
    ...
    def on_train_begin(self, args, state, control, **kwargs):
        ...
        self._write("training", 0, state.max_steps or 1)
    ...
    def on_train_end(self, args, state, control, **kwargs):
        self._write("training_done", state.max_steps, state.max_steps or 1)
...
def main():
    ...
    def apply_audio_decoder(batch, rank=None):
        ...
        # Pass RAW (undelayed) codes as labels: shape (1, 1, num_codebooks, seq_len)
        # -> (num_codebooks, seq_len), matching what the collator/model expect.
        #
        # BUG FIX: this function used to also call build_delay_pattern_mask +
        # apply_delay_pattern_mask on the labels here. But MusicgenForCausalLM.
        # forward() already does that shift internally whenever `labels` is
        # passed without explicit input_ids/decoder_input_ids (see its
        # docstring: "labels are shifted inside the model, i.e. you can set
        # labels = input_ids"). Pre-applying the delay pattern here as well
        # meant the targets got double-transformed -- the model's predictions
        # were being scored against a target sequence structurally misaligned
        # from what it could ever produce. This was confirmed empirically: an
        # overfit test on 160 repeated examples for 30 epochs plateaued at a
        # loss ~7x *above* the random-guessing baseline (ln(2048) =~ 7.6),
        # which is only explainable by corrupted/misaligned targets, not
        # insufficient training.
        batch["labels"] = labels.squeeze(0).squeeze(0).cpu()
        return batch
    ...
    # Only load CLAP when eval is actually requested -- its checkpoint is
    # pickle-format only, which newer transformers blocks without torch>=2.6.
    if training_args.do_eval:
    ...
    if model_args.use_lora:
        ...
        # Deliberately NOT touching lm_heads / embed_tokens this time: two prior
        # runs that adapted them showed loss rising (38.4 -> 45.3 -> 51.2) and
        # output degrading from "hot garbage" to "unintelligible mumbled
        # textures" as training progressed -- lm_heads governs next-token
        # (i.e. musical-coherence) prediction directly, so destabilizing it
        # plausibly explains "nice tone/texture but not coherent music".
        # Keeping only attention + feedforward, the standard conservative
        # LoRA footprint.
        target_modules = [
            "enc_to_dec_proj",
    ...
    trainer = MusicgenTrainer(
        ...
        callbacks=[DashboardProgressCallback()],
    )
